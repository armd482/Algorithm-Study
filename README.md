...
따라서, 처음에 입력받은 수 만큼의 크기를 갖는 -1이 입력된 리스트를 만든 후, 입력받은 수들의 index를 스택에 저장.
저장된 index에 대해서, 해당 값보다 큰 수가 나올 경우, 스택에 들어있는 해당 index를 제거한 후, 큰 수를 -1이 저장된 리스트의 해당 index에 저장.
* * *
